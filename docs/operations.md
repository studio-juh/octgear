...
Hardware設定モーダルの「レイヤー表示」は、Layer色による全灯、4灯の点灯パターン、虹色サイクルを切り替えます。4灯パターンでは点灯pixelに現在Layerの色を使い、消灯pixelを含めてLayer切り替え時に200 msで遷移します。虹色サイクルではLayer色の代わりに4灯へ異なる色相を表示して流します。設定は変更時にFlashへ即座に保存されます。
...
Hardware設定モーダルの「アニメーション効果」は、打鍵表示を無効／波紋／フラッシュ／スパーク／打鍵虹色から選ぶプルダウンです。変更時に実機へ即座に保存します。波紋は押下位置から左右へ広がり、フラッシュは4灯を同時点灯、スパークは押下位置とその周囲を短く光らせます。打鍵虹色は4灯へ異なる色相を表示し、押すたびに開始色相を変えながら320 msで通常表示へ戻ります。通常の3効果は現在または切り替え後のLayer色で発光します。EncoderのCCW / CW / SWは右端のLED 4をK4 / K8と共有します。
...
| `animation <0-4>` | 打鍵効果を波紋／無効／フラッシュ／スパーク／打鍵虹色に設定 | Yes |
| `animation-brightness <0-128>` | 打鍵アニメーションの輝度上限を設定 | Yes |
| `layer-display <0-2>` | Layer表示を全灯／4灯パターン／虹色サイクルに設定 | Yes |
...
`animation`は`0`が波紋、`1`が無効、`2`がフラッシュ、`3`がスパーク、`4`が打鍵虹色です。`layer-display`は`0`が全灯、`1`が4灯パターン、`2`が虹色サイクルです。`pcb-revision`は`2`または`3`だけを受け付けます。boolean設定は`0`が標準／無効、`1`が反転／有効です。
