...
- `LayerCycle`は最初の押下を設定されたTap Dance判定時間だけ保留し、単押しなら有効なlayerだけを次方向へ循環します。判定時間は50-1000 ms、既定250 msでFlashへ保存します。同じ物理controlを期限内に再度押すと保留した次方向への遷移を破棄し、開始位置から`LayerPrevious`相当として前方向へ循環します。別controlの押下で中断された場合は、保留した次方向への遷移を先に確定してから移動先layerのassignmentを解決します。
