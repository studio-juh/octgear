...
5. 通常: active layer色 + 選択された打鍵アニメーション

PCのresume後は現在Layerの色へ戻ります。Layer色は200 ms以内で遷移し、打鍵アニメーションは無効／波紋／フラッシュ／スパークから選択します。通常打鍵のハイライトは白色、Layer変更を伴うcontrolは切り替え後のLayer色を使います。
