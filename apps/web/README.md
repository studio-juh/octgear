...
2. HID deviceをopen
3. heartbeatを送信
4. `GetState`でdevice dimensions、layer状態、Encoder方向、LEDテープ方向、打鍵アニメーションを取得
...
Read左のoverflow menuから初期化を選ぶと、確認dialogを経て全assignment、layer有効状態、RGB色、Encoder方向、LEDテープ方向、打鍵アニメーションをfirmwareの既定値へ戻し、実機へ保存してからUIを再読込します。
